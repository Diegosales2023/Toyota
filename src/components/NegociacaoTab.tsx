...
export default function NegociacaoTab() {
  ...
  const handleNegociacaoSubmit = (e: React.FormEvent) => {
    ...
    if (contactMethod === 'phone') {
      ...
      const waMessage = `Olá, gostaria de solicitar uma proposta de negociação/acordo para o meu contrato:
...
- Telefone de Contato: ${telefone}`;
      ...
      const emailSubject = `Solicitação de Acordo/Negociação de Financiamento - ${nome}`;
      const emailBody = `Olá, gostaria de solicitar uma proposta de negociação/acordo para o meu contrato:
- Nome do Titular: ${nome}
- CPF/CNPJ: ${cpf}
- E-mail de Contato: ${email}
- Telefone de Contato: ${telefone}
        ...
        {submitted ? (
          <div className="lg:col-span-5 bg-white border border-gray-100 shadow-md rounded-2xl p-6 sm:p-8 space-y-6 text-center py-12">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                {contactMethod === 'phone' ? (
                  <>Seus dados foram verificados com sucesso. Um especialista de negociação entrará em contato com você no telefone <strong className="text-slate-800">{telefone}</strong> para apresentar nossa melhor proposta de renegociação de atrasos.</>
                  ...
                  <>Sua solicitação foi gerada com sucesso! O rascunho de e-mail para acordo/negociação de financiamento foi aberto. Envie a mensagem diretamente para <strong className="text-slate-800">atendimento@bancotoyota.com.br</strong>.</>
                )}
              </p>
            </div>
            <div className="pt-4 border-t border-slate-100 space-y-3">
              {contactMethod === 'phone' ? (
                ...
                <>
                  ...
                  <a
                    href={`https://api.whatsapp.com/send?phone=5511977655148&text=${encodeURIComponent(`Olá, gostaria de solicitar uma proposta de negociação/acordo para o meu contrato:\n- Nome: ${nome}\n- CPF/CNPJ: ${cpf}\n- E-mail: ${email}\n- Telefone: ${telefone}`)}`}
                    target="_blank"
                ...
                <>
                  ...
                  <a
                    href={`mailto:atendimento@bancotoyota.com.br?subject=Solicitação de Acordo/Negociação - ${nome}&body=${encodeURIComponent(`Olá, gostaria de solicitar uma proposta de acordo/negociação para o meu contrato:\n- Nome do Titular: ${nome}\n- CPF/CNPJ: ${cpf}\n- E-mail de Contato: ${email}\n- Telefone de Contato: ${telefone}\n\nAtenciosamente,\n${nome}`)}`}
                    className="w-full py-3.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-xs font-bold shadow-md shadow-red-600/20 transition-all flex items-center justify-center space-x-2 cursor-pointer border-none outline-none text-center"
                  >
          ...
          <div className="lg:col-span-5 bg-white border border-gray-100 shadow-md rounded-2xl overflow-hidden p-6 sm:p-8 space-y-6">
            <div className="space-y-2">
              ...
              <h3 className="text-xl font-bold text-gray-900">Negociação de Parcelas</h3>
              <p className="text-xs text-slate-500 leading-relaxed">
                Escolha o canal de preferência e preencha os dados cadastrais do seu financiamento abaixo para receber uma proposta de renegociação com taxas reduzidas.
              </p>
            </div>

            {/* Canal Selection Tabs */}
            <div className="flex rounded-xl bg-slate-100/80 p-1 gap-1" id="negociacao-contact-tabs">
              <button
                type="button"
            ...
            <form onSubmit={handleNegociacaoSubmit} className="space-y-4 pt-2">
              <div className="space-y-1.5">
                ...
                >
                  {contactMethod === 'phone' ? (
                    <>
                      ...
                      <span>Solicitar Proposta por Telefone</span>
                    </>
                  ) : contactMethod === 'whatsapp' ? (
                    <>
                      <MessageSquare className="h-4.5 w-4.5 shrink-0" />
                      <span>Solicitar Proposta por WhatsApp</span>
                    </>
                  ) : (
                    <>
