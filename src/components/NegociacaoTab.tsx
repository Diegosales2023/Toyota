...
import { Handshake, Percent, Scale, HelpCircle, Info, ShieldAlert, FileText, CheckCircle2 } from 'lucide-react';

export default function NegociacaoTab() {
  const handleWhatsAppRedirect = (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    const url = 'https://api.whatsapp.com/send?phone=5511977655148&text=Solicito%20Atendimento%20-%20Negociacao%20de%20Parcelamento';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 bg-white border border-emerald-100 shadow-md rounded-2xl overflow-hidden p-6 sm:p-8 space-y-6">
          <div className="space-y-2">
            ...
            <h3 className="text-xl font-bold text-gray-900">Negociação via WhatsApp</h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              Consulte seu CPF ou CNPJ e inicie o parcelamento de parcelas atrasadas ou renegociação de contrato com condições especiais da Kira.
            </p>
          </div>

          <div className="space-y-4 border-t border-slate-100 pt-4">
            <h4 className="font-bold text-xs text-slate-800">Como funciona o atendimento de renegociação:</h4>
            
            <ul className="space-y-2.5 text-xs text-slate-600">
              <li className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-emerald-500 shrink-0" />
                <span>Simulação imediata de planos de parcelamento</span>
              </li>
              <li className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-emerald-500 shrink-0" />
                <span>Descontos de até 90% em encargos moratórios</span>
              </li>
              <li className="flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-emerald-500 shrink-0" />
            ...
            <div className="p-3 bg-emerald-50/50 border border-emerald-100/40 rounded-xl text-[11px] text-emerald-800">
              ...
              <p>Evite restrições e ligações de cobrança regularizando seu contrato hoje mesmo.</p>
            </div>
          </div>
          ...
          >
            ...
            <span>Negociar Parcelas no WhatsApp</span>
          </button>
        </div>
      </div>
    </div>
